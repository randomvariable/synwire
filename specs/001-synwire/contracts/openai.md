# Provider Contract: synwire-openai
...
impl From<OpenAIError> for SynwireError {
    fn from(e: OpenAIError) -> Self {
        match e {
            OpenAIError::ParseError(msg) => SynwireError::ParseError(msg),
            _ => SynwireError::ModelError(e.to_string()),
        }
    }
}
```
...
- **`AzureChatOpenAI` / `AzureOpenAIEmbeddings`**: Azure-hosted OpenAI
  ...
  deployment names). This is a separate provider crate (`synwire-azure-openai`),
  not part of the initial `synwire-openai` crate.
...
- **`OpenAIModerationMiddleware`**: Content moderation via OpenAI's
  ...
  `synwire-openai::moderation` — wraps as a `RunnableLambda` that
...
1. Add `synwire-core` as a dependency
...
4. Define provider-specific error types with `From<YourError> for SynwireError`
5. Use builder pattern for configuration
6. Feature-gate integration tests behind `integration-tests`
7. See `synwire-openai` as the reference implementation

### Provider Testing Strategy

- **Unit tests**: Use `FakeChatModel` and `FakeEmbeddings` from `synwire-core`
